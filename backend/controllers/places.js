...
router.post("/:placeId/comments", async (req, res) => {
  ...
  // if (!req.currentUser){
  //   return res.status(404).json({message: `You must be logged in to leave rant or rave.`})
  // }
  ...
  if (!req.currentUser) {
    return res.status(404).json({
      message: "Hey dumbie, you must me logged in to leave a RANT or RAVE",
    });
  }
...
router.delete("/:placeId/comments/:commentId", async (req, res) => {
  ...
  if (isNaN(placeId)) {
    ...
    if (!comment) {
      ...
      res.status(403).json({
        message: `You DO NOT have permission to delete this comment: ${comment.commentId}`,
      });
    } else {
